...
public class PassiveIsologismosFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        String InputKefalaio = editTextKefalaio.getText().toString();
        String InputProvlepseis = editTextProvlepseis.getText().toString();
        String InputMYpo =  editTextM_YPO.getText().toString();
        String InputBYpo = editTextB_Ypo.getText().toString();

        if(!InputKefalaio.isEmpty()){
            if(!InputProvlepseis.isEmpty()) {
                if(!InputMYpo.isEmpty()) {
                    if(!InputBYpo.isEmpty()) {
                        kefalaio = Float.parseFloat(InputKefalaio);
                        provlepseis = Float.parseFloat(InputProvlepseis);
                        m_ypo = Float.parseFloat(InputMYpo);
                        b_ypo = Float.parseFloat(InputBYpo);
                        ...
                        Toast.makeText(requireContext(),"Braxyxronies Ypoxrewseis cannot be empty",Toast.LENGTH_SHORT).show();
                    }
                }else{
                    Toast.makeText(requireContext(),"Makroxronies Ypoxrewseis cannot be empty",Toast.LENGTH_SHORT).show();
                }
            }else{
                Toast.makeText(requireContext(),"Provlepseis cannot be empty",Toast.LENGTH_SHORT).show();
            }
        }else{
            Toast.makeText(requireContext(),"Kefalaio cannot be empty",Toast.LENGTH_SHORT).show();
        }
        return view;
    }
